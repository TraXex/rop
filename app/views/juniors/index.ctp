...
foreach ($posts as $post) {
    ...
    <div class="sos_div <?php if($post['PostDetail']['type']!='sos') echo "content-div"; ?>">
        ...
        <div class="info">
            ...
            <div class="content">
                ...
                echo $this->Html->link($post['Post']['topic'], array('controller' => 'juniors', 'action' => 'view_news', $post['Post']['id']),array('escape' => false));
            } 
                else{
    ...
    $i++;
                ...
                <?php
            if ($post['PostDetail']['type'] == 'sos') {
                echo $this->Html->link('view more replies', array('controller' => 'juniors', 'action' => 'view_sos', $post['Post']['id']), array('escape' => false));
            } elseif ($post['PostDetail']['type'] == 'expert advice') {

                echo $this->Html->link('view more comments', array('controller' => 'juniors', 'action' => 'view_advice', $post['Post']['id']),array('escape' => false));
            }
            elseif ($post['PostDetail']['type'] == 'pink up') {
                echo $this->Html->link('view more replies', array('controller' => 'juniors', 'action' => 'view_pink_me_up', $post['Post']['id']),array('escape' => false));
            }elseif ($post['PostDetail']['type'] == 'news') {
                echo $this->Html->link('view more comments', array('controller' => 'juniors', 'action' => 'view_news', $post['Post']['id']),array('escape' => false));
            } 
            else {

               
                echo $this->Html->link('view more comments', array('controller' => 'juniors', 'action' => 'view', $post['Post']['id']));
            }
                ?>
            </div>
            <?php } else {?>
    ...
    $i++;
        ...
        <?php } ?>
            ...
            <div class="option-menu">
                ...
                echo $this->Html->image("comment-icon.png", array("alt" => "comment-icon",'class'=>'comment target','title'=>$comments, 'url' => array('controller' => 'juniors', 'action' => 'view_news', $post['Post']['id'])));
            } 
            else {
             ...
             ?></li>
                       <li><?php
                        if ($post['PostDetail']['type']== 'sos'){
                                ...
                                echo $this->Html->image("icon-02.png", array("alt" => "view-icon",'class'=>'view target','title'=>$post['PostDetail']['total_views'], 'url' => array('controller' => 'juniors', 'action' => 'view_news', $post['Post']['id']))); 
                        }
                        else{
